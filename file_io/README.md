...
**Answer:** `O_WRONLY | O_CREAT | O_APPEND`
...
**Answer:** `it's a function provided by the kernel`
...
**Question:** What system call would you use to write to a file descriptor?
...
**Answer:** `The text will be printed on the terminal on the standard output`
...
- [`0-read_textfile.c`](https://github.com/Antgst/holbertonschool-low_level_programming/blob/main/file_io/0-read_textfile.c)
...
if
write
fails or does not write the expected amount of bytes, return
0
julien
@ubuntu
:
~/
File
descriptors and permissions$ cat
Requiescat
Requiescat
by
Oscar
...
~/
File
descriptors and permissions$ gcc
-
Wall
-
pedantic
...
- [`1-create_file.c`](https://github.com/Antgst/holbertonschool-low_level_programming/blob/main/file_io/1-create_file.c)
...
where
filename
is the name of the file to create and
text_content
is a
NULL
...
julien@ubuntu:~/File descriptors
and
permissions$ cat
1
-main.c
...
julien@ubuntu:~/File descriptors
and
permissions$ gcc -Wall -pedantic -Werror -Wextra -std=gnu89
1
-main.c
1
-create_file.c -o b
julien@ubuntu:~/File descriptors
and
permissions$ ./b hello world
->
1
)
julien@ubuntu:~/File descriptors
and
permissions$ ls -l hello
-rw-------
1
julien julien
5
...
julien@ubuntu:~/File descriptors
and
permissions$ cat hello
worldjulien@ubuntu:~/File descriptors
and
permis$
Repo:
GitHub repository:
...
- [`2-append_text_to_file.c`](https://github.com/Antgst/holbertonschool-low_level_programming/blob/main/file_io/2-append_text_to_file.c)
...
is
NULL
, do not add anything to the file. Return
1
...
~/
File
descriptors and permissions$ echo
-
n
Hello
>
...
~/
File
descriptors and permissions$ ls
-
l hello
-
rw
...
~/
File
descriptors and permissions$ cat hello
Hello
World
!
julien
...
- [`3-cp.c`](https://github.com/Antgst/holbertonschool-low_level_programming/blob/main/file_io/3-cp.c)
...
You must read
1,024
bytes at a time from the
file_from
to make less system calls. Use a buffer
You are allowed to use
dprintf
julien@ubuntu:~
/File descriptors and permissions$ gcc -Wall -pedantic -Werror -Wextra -std=gnu89 3-cp.c -o cp
julien@ubuntu:~/
File descriptors
and
permissions$ cat incitatous
...
julien@ubuntu:~
/File descriptors and permissions$ ls -l Incitatous
